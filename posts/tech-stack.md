...
여러 블로그 플랫폼을 고민하다가, 직접 만들어 보는 것도 재밌겠다 싶어서 깃허브 블로그를 선택하게 되었다. 구글링을 통해 어떤 방식으로 깃허브 블로그를 만드는 지 검색해 보았는데 정말 다양한 프레임워크 조합들이 있었다. 그 중에서  현재 가장 익숙한 Next.js + GitHub Pages 조합으로 진행했다. 결과적으로 하루 만에 블로그를 완성했지만, 그 과정에서 꽤 많은 삽질이 있었다.
...
약 1시간 동안 6분 간격으로 설정을 수정하고 배포하는 걸 반복했다.
