...
마침 SEO도 손볼 타이밍이었다. [6편: 정적 블로그에 다국어(i18n) 붙이기](/ko/posts/i18n-static-blog)에서 `hreflang`과 sitemap 교차 참조를 다뤘지만, 포스트 단위의 구조화 데이터(JSON-LD)나 Open Graph 메타데이터는 아직 없었다. 복사 방지와 SEO, 두 작업을 같이 진행했다.
