...
export default function Comments({ slug }: { slug: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await postComment(slug, nickname, content, password).catch(
      () => ({ success: false, error: "네트워크 오류가 발생했습니다." })
    );
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="flex gap-2">
          <input
            ...
            maxLength={30}
            required
            className="w-32 sm:w-40 px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
          />
          <input
            type="password"
            inputMode="numeric"
            ...
            maxLength={4}
            required
            className="w-32 sm:w-36 px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
          />
        </div>
        <textarea
