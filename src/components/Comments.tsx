...
function CommentForm({
  ...
  const isReserved = reserved.some((r) => nickname.trim().toLowerCase() === r.toLowerCase());
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await postComment(slug, nickname, content, password, parentId, isReserved ? adminToken : undefined).catch(
    ...
    <form onSubmit={handleSubmit} className="space-y-2">
      ...
      {isReserved && (
        <input
          ...
          required
          className={`${compact ? "w-48 sm:w-64" : "w-48 sm:w-64"} px-3 py-2 text-sm rounded-lg border border-amber-300 dark:border-amber-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent`}
        />
      )}
      <textarea
